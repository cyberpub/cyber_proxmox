...
install_tailscale() {
    ...
    echo -e "\033[0;32m${log_prefix}\033[0m Installing Tailscale using official installer..."
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "  • For exit node: sudo tailscale up --advertise-exit-node"
        echo "  • Check status: tailscale status"
        echo "  • Get your IP: tailscale ip"
        return 0
    else
